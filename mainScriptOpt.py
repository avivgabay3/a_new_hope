...
def compress_and_remove(filepath):
    ...
    zip_path = filepath.replace(".avi", ".zip")
    try:
        # Transcode using ffmpeg
        os.system(f'ffmpeg -y -i "{filepath}" -vcodec libx264 -crf 28 -preset veryfast "{mp4_path}"')

        os.remove(filepath)  # Remove original .avi
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            zipf.write(filepath, arcname=os.path.basename(filepath))
        print(f"Compressed and saved: {mp4_path}")
